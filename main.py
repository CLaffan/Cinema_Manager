...
_customers = []
cbCustomer = Combo(bookbox, options= _customers, visible= True,
                   align="left", grid=[1, 0])
cbCustomer.bg = "palegreen3"

Text(bookbox, text="Session:", grid=[0, 1])
#set session selection
cbSession = Combo(bookbox, options=range (1, 7), align="left", grid=[1, 1])
cbSession.bg = "palegreen3"

Text(bookbox, text="Movies:", grid=[0, 2])
#set selection of movies from a list
cbMovies = Combo(bookbox, options=range (7, 21),
                 align="left", grid=[1, 2], enabled= True, visible=True)
cbMovies.bg = "palegreen3"

btnbox = Box(frmBooking, width="fill", align="top",
             border=True, layout="auto")

#set new button event
